...
| ObjectStoragePolicy | Object storage policy document | `status.name` | none | Identity is `(serviceUUID, name)`. The policy document is immutable in v0.1 (the API only offers versioning); a change requires recreation. Deletion is refused (409) while the policy is attached to a user. In a policy document, `Resource: "*"` is verified against the real API (the e2e sample uses it). An invented namespace such as `arn:upcloud:objectstorage::...` is rejected with 400 `Policy has invalid resource`. Standard S3 ARNs (`arn:aws:s3:::bucket`, `arn:aws:s3:::bucket/*`) are expected to work but are not yet verified against this endpoint; UpCloud's examples for them come from S3-API bucket policies, not the IAM policy endpoint. `*` grants access to every bucket in the service, so narrow it in real use. |
...
`spec.labels` on a kind that supports it is written to the UpCloud resource, not to the Kubernetes object. The operator always sets its own `managed-by` and `k8s-uid` labels; a user value for either is ignored, so adoption (which matches on `k8s-uid`) cannot be hijacked.

| Label | Value | Overridable |
|---|---|---|
| `managed-by` | `uck` | no |
| `k8s-uid` | the CR's `metadata.uid` | no |

UpCloud limits label keys to 32 characters and forbids a leading `_`, so label keys must stay short.
